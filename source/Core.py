...
class BaseCRUD:
    ...
    @classmethod
    async def model_update(cls, model_id: int, **arg):
        async with async_session() as session:
    ...
    @classmethod
    async def model_delete(cls, model_id: int):
        async with async_session() as session:
...
class BaseJWT:
    scheme_validator: Type[BaseModel] = None

    @classmethod
    def decode_token(cls, token: str) -> Type[BaseModel]:
        try:
            payload: dict = decode(
                jwt=token,
...
class BaseSMTPClient:
    @classmethod
    def send_mail(cls, email: EmailStr, body, subject):
